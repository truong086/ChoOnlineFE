    ...
    <div class="container-scroller">
      ...
      <nav class="sidebar sidebar-offcanvas" id="sidebar">
        <div class="sidebar-brand-wrapper d-none d-lg-flex align-items-center justify-content-center fixed-top">
          <a class="sidebar-brand brand-logo" href="../../index.html"><img src="../../assets/FromSeller/images/logo.svg" alt="logo" /></a>
          <a class="sidebar-brand brand-logo-mini" href="../../index.html"><img src="../../assets/FromSeller/images/logo-mini.svg" alt="logo" /></a>
        </div>
        <ul class="nav">
            ...
            <ul class="navbar-nav navbar-nav-right">
              ...
              <li class="nav-item dropdown border-left">
                ...
                <div class="dropdown-menu dropdown-menu-right navbar-dropdown preview-list" aria-labelledby="messageDropdown">
                  ...
                  <a class="dropdown-item preview-item">
                    <div class="preview-thumbnail">
                      <img src="../../assets/FromSeller/images/faces/face3.jpg" alt="image" class="rounded-circle profile-pic">
                    </div>
                    <div class="preview-item-content">
              ...
              <div class="col-md-6 grid-margin stretch-card">
                <div class="card">
                  <div class="card-body">
                    ...
                    <form class="forms-sample" style="width: 700px">
                      <div class="form-group">
                        <label for="exampleInputUsername1">title</label>
                        <input type="text" v-model="product.title" class="form-control" id="exampleInputUsername1" placeholder="title">
                        <input type="hidden" v-model="product.id" class="form-control" id="exampleInputUsername1" placeholder="title">
                      </div>
                      <div class="form-group">
                        <label for="exampleInputEmail1">Description</label>
                        <input type="email" v-model="product.description" class="form-control" id="exampleInputEmail1" placeholder="description">
                      </div>
                      <div class="form-group">
                        <label for="exampleInputPassword1">price</label>
                        <input type="number" v-model="product.price" class="form-control" id="exampleInputPassword1" placeholder="price">
                      </div>
                      <div class="form-group">
                        <label for="exampleInputConfirmPassword1">Lượt xem</label>
                        <input type="text" v-model="product.click" class="form-control" id="exampleInputConfirmPassword1" placeholder="Password" readonly>
                      </div>
                      <div class="form-group" v-if="checkRole">
                        <label for="exampleInputConfirmPassword1">Account</label>
                        <select v-model="product.account_id">
                          <option v-for="(item, index) in shop" :key="index" :value="item.id">{{ item.name }}</option>
                        </select>
                      </div>
                      <div class="form-check form-check-flat form-check-primary">
                        <label class="form-check-label">Image</label>
                        <input type="file" ref="fileInput" @change="handleImageUpload" multiple />
                        <div>
                          <Listdata :data="dataImage" @loadDataImage="loadDataImage" @RemoveImage="removeImage"></Listdata>
                        </div>
                        
                      </div>
                      <div class="form-check form-check-flat form-check-primary" >
                        <div v-for="(check, index) in categoryList" :key="index">
                          <label>
                              <input type="checkbox" v-model="check.isSelect"/>
                                  {{ check.name }}
                                </label>
                        </div>
                        
                      </div>
                      <button type="button" class="btn btn-primary me-2" v-on:click="AddOrEdit(product.id)">Gửi</button>
...
import Listdata from '../PageList/ListImagePage.vue'
...
export default{
  ...
  data(){
    return {
      ...
      imageFiles: [], // Danh sách các tệp ảnh đã chọn
      // imageByteArrays: [], // Mảng mảng byte
      imageBase64Array: [], // Mảng chuỗi Base64
      categoryList: [],
      dataListImage: [],
      shop: []
    }
  ...
  async created() {
    if(this.$route.params.type === "Edit" && this.$route.params.table === "Product") await this.FindOneProduct()
    else await this.ResetImage()
    await this.FindAllCategory()
    await this.FindAllShop()
  },
  computed:{
    ...mapState(['dataImage']), // Lấy ra những biến trong lớp "getters" trong "store.js", ở đây đang là biến "dataImage"
    ...mapGetters(['imageLeng', 'laydanhsachRole']),// Lấy ra những thuộc tính trong lớp "getters" trong "store.js", ở đây đang là thuộc tính "imageLeng"
    imageLengs(){
      return this.imageLeng
    },
    checkRole(){
      return this.laydanhsachRole(0) === 'Admin'
    }
  }
  ,
  methods: {
    ...
    //   /**
    //    * Xử lý việc chọn tệp ảnh:
    //      "const fileInput = this.$refs.fileInput": Lấy tham chiếu đến phần tử <input type="file"> từ giao diện người dùng.
    //      "this.imageFiles = Array.from(fileInput.files)": Chuyển đổi danh sách tệp đã chọn thành một mảng JavaScript, 
    //      lưu trữ trong this.imageFiles.
    //    */
    //   const fileInput = this.$refs.fileInput
    //   this.imageFiles = Array.from(fileInput.files)
      

    //    // Đọc và chuyển thành mảng byte
    //    /**"this.imageByteArrays = this.imageFiles.map((file) => { ... })": Sử dụng phương thức map để duyệt qua từng tệp ảnh 
    //     * trong this.imageFiles. */
    //    this.imageByteArrays = this.imageFiles.map((file) =>{
    //     /**
    //      * Trong mỗi vòng lặp:
    //       - Tạo một "Promise" để thực hiện việc đọc và chuyển đổi tệp ảnh thành mảng byte.
    //      */
    //     return new Promise((resolve) => {
    //       // "const reader = new FileReader()": Tạo một đối tượng FileReader để đọc tệp.
    //       const reader = new FileReader()
    //       reader.onload = (e) =>{ // "reader.onload": Sự kiện được kích hoạt khi quá trình đọc tệp hoàn tất.
    //         /**
    //          * Trong sự kiện onload:
    //           "const arrByte = new Uint8Array(e.target.result)": Chuyển dữ liệu từ sự kiện onload thành mảng byte (Uint8Array) 
    //           và lưu trữ trong arrByte.
    //          */
    //         const arrByte = new Uint8Array(e.target.resul)
            
    //         // "resolve(arrByte)": Đưa mảng byte này vào trong Promise để hoàn thành việc đọc và chuyển đổi tệp.
    //         resolve(arrByte)
    //       }

    //       // Giải thích đoạn code "reader.readAsArrayBuffer(file)"
    //       // "reader.readAsArrayBuffer(file)" thực hiện việc đọc tệp ảnh đã được chọn dưới dạng mảng byte.
    //       /**
    //        * "reader": Là một đối tượng FileReader được sử dụng để đọc nội dung của các tệp được chọn.
    //         "readAsArrayBuffer(file):" Là phương thức của FileReader để bắt đầu quá trình đọc tệp dưới dạng mảng byte.

    //         Khi gọi reader.readAsArrayBuffer(file), FileReader bắt đầu đọc nội dung của tệp file đã được chọn. 
    //         Khi việc đọc hoàn tất, sự kiện onload của FileReader sẽ được kích hoạt, và bạn có thể truy cập dữ liệu
    //         đã được đọc thông qua thuộc tính e.target.result trong hàm xử lý sự kiện onload.

    //         Trong trường hợp này, "readAsArrayBuffer(file)" đọc tệp ảnh đã chọn dưới dạng mảng byte (ArrayBuffer), 
    //         và khi quá trình đọc hoàn tất, sự kiện onload sẽ được kích hoạt để bạn có thể thực hiện xử lý tiếp theo
    //          trên dữ liệu của tệp ảnh.
    //        */
    //       reader.readAsArrayBuffer(file)
    //     })
         
    //    })
    //    console.log(this.imageByteArrays)
    //   this.uploadImages()
       
    // },
    // async uploadImages(){
    //   this.imageBase64Array = await Promise.all(
    //     this.imageByteArrays.map(async (byteArray) => {
    //       const base64 = btoa(
    ...
    // }
    // ,
    ...mapMutations(['updateImage']), // Lấy ra những hàm trong lớp "mutations" trong "store.js", ở đây đang là hàm "updateImage"
    ...mapActions(['AddDataImage', 'RemoveImage', 'ResetImage']), // Lấy ra những hàm trong lớp "actions" trong "store.js", ở đây đang là hàm 'AddDataImage', 'RemoveImage', 'ResetImage'
    ...
    handleImageUpload() {
      const fileInputs = this.$refs.fileInput;
      console.log(this.imageLengs)
      if(fileInputs.files.length > 0 && fileInputs.files.length < 9 && this.imageLeng < 8 && fileInputs.files.length + this.imageLeng < 9){
        this.imageFiles = Array.from(fileInputs.files);
        const promises = this.imageFiles.map(file => this.convertToBase64(file));

        Promise.all(promises)
          .then((results) => {
            this.imageBase64Array = results;
            // this.updateImage(this.imageBase64Array)
            this.AddDataImage(this.imageBase64Array) // Cách 2: Lưu 1 list danh sách mới vào list danh sách cũ trước đó
            // this.dataListImage = [...this.dataListImage, ...this.imageBase64Array] // Cách 1: Lưu 1 list danh sách mới vào list danh sách cũ trước đó
            // this.imageBase64Array.push(results)
            this.loadDataImage()
          })
          .catch((error) => {
            console.error(error);
    ...
    async FindAllShop(){
      this.checkLoad = true
      const res = await axios.get(this.hostname + '/api/Shop/findAll?page=1&pageSize=20', this.getTokenConfig())
      this.shop = res.data.content.data
      this.checkLoad = false
      if(res.data.errorCode !== 200){
        this.$notify({
              title: "Thông báo",
              text: "Lỗi",
              duration: 5000, // Thời gian hiển thị (tùy chọn)
              type: 'error' // Loại thông báo (tùy chọn)
            });
            return
      }
    ...
    async AddOrEdit(id){
      this.checkLoad = true
      const nameUser = this.$store.getters.name
        if(this.dataImage.length <= 0){
          Swal.fire("Vui lòng chọn ảnh")
          return
        }
        this.product.image = this.dataImage
        let arr = []
        ...
        if(arr.length <= 0){
          Swal.fire("Chưa chọn thể loại")
          return
        }
        this.product.category = arr
        let idP = this.product.id
      if(id > 0){
        this.checkLoad = true
        this.product.account_name = nameUser
        await axios.put(this.hostname + `/api/Product/EditProduct?id=${id}&name=${nameUser}`, this.product, this.getTokenConfig())
          .then(response => {
            // Hiển thị thông báo
            // this.$notify({
            //   title: "Thông báo",
            ...
            //   duration: 5000, // Thời gian hiển thị (tùy chọn)
            //   type: 'success' // Loại thông báo (tùy chọn)
            // });
            console.log(response)
            this.ResetImage()
            // Swal.fire("Success")
            // Toasted.show('Đây là thông báo của bạn!', {
            //   duration: 3000 // Thời gian hiển thị thông báo (3 giây)
            // })
              this.product.id = idP
              this.checkLoad = false
            localStorage.setItem('thongbao', JSON.stringify({
              title: 'Thông báo',
              text: 'Edit thành công',
              type: 'success',
        ...
        this.product.account_name = name
        await axios.post(this.hostname + `/api/Product/AddProduct`, this.product, this.getTokenConfig())
          .then(response => {
    ...
    async FindAllCategory(){
      this.checkLoad = true
      await axios.get(this.hostname + `/api/Category/FindAll`, this.getTokenConfig())
        .then(response => {
          let addIsSelect = []
          addIsSelect = response.data.content.data
          this.categoryList = addIsSelect.map(x => {
            return {...x, isSelect: false}
          })
          

          this.product.category.forEach(id => {
            var checkId = this.categoryList.find(x => x.name === id)
            if(checkId){
              checkId.isSelect = true
            }
          })
          this.checkLoad = false
    ...
    async FindOneProduct(){
      ...
      if(localStorage.getItem('IdProduct') !== null){
        id = JSON.parse(localStorage.getItem('IdProduct'))
      }else{
        id = this.product.id
      }
      ...
      await axios.get(this.hostname + `/api/Product/FindOneId?id=${id}`, this.getTokenConfig())
        .then(response => {
          
          this.product = response.data.content
          // this.dataListImage = response.data.content.image
          this.updateImage(response.data.content.image)
          this.checkLoad = false
          console.log(response)
        })
        .catch(error => {
